...
void applicationStartup(struct AccountTicketingData *data)//(struct Account* accounts, int maxElement)
{
	int index_num;
	do
	{
		index_num = menuLogin(data->accounts, data->ACCOUNT_MAX_SIZE);//(accounts, maxElement);
		if (index_num != -1)
		{
			if(data->accounts[index_num].account_type == 'A')//if (accounts[index_num].account_type == 'A')
			{
				menuAgent(data, &data->accounts[index_num]);//menuAgent(accounts, maxElement, &accounts[index_num]);
			}
...
void menuAgent(struct AccountTicketingData *acc_data, const struct Account* account_info)//(struct Account* account_Data, int maxElement, const struct Account* account_info)
{
